...
class MetaCollection(type):
    ...
    def __new__(cls, name, bases, namespace, **kwds):
        ...
        if dbsystem in implementedDatabases:
            ...
            try:
                databaseModule = importlib.import_module('framework.database.' + dbsystem + 'Collection')
            except:
                raise ImportError('The module %s does not exist' % dbsystem)
        else:
            raise NotImplementedError("The %s database system does not implemented yet" % dbsystem)
